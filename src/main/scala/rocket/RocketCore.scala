...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  println(s"useRVE:${coreParams.useRVE}")
  ...
  //****************************************************************************
  val nulctrl = Module(new freechips.rocketchip.nulctrl.NulCPUCtrlWithUart(100000000,1000000))
  ...
  val _nul_invoke_nowait = RegInit(false.B)
  ...
  val _take_pc_target = Mux(wb_xcpt || csr.io.eret, csr.io.evec, // exception or [m|s]ret
                        Mux(replay_wb,              wb_reg_pc,   // replay
                                mem_npc))    // flush or branch misprediction
  when(_nul_invoke_working && take_pc && _take_pc_target === _nul_fake_pc) {
    _nul_invoke_valid := true.B 
  }
  ...
  _nul_ifu_invoke_data.data := _nul_invoke_inst
  _nul_ifu_invoke_data.pc := _nul_fake_pc
  ...
  when(_nul_invoke_valid && ibuf.io.imem.ready) {
    _nul_invoke_valid := false.B 
  }
  ...
  io.nul_port.nul_current_pos := 0.U
  io.nul_port.nul_push_pos := 0.U
  io.nul_port.nul_reg_flush0 := 0.U
  io.nul_port.nul_state := 0.U
